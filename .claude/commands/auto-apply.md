...
2. **Broad discovery (bounded, cheap).** Call the JobDataLake MCP `search_jobs` tool a small number of times — roughly one call per resume category in `config/resumes.json` (currently software-engineer, backend-developer, ai-engineer), filtered to US + remote and a salary floor pulled from `config/profile.local.json`'s state-tier `DEFAULT.low`. This is O(query count), not O(job count) — do not call it per job. Pipe the combined results into `npx tsx scripts/import-jobs.ts` (pass the JSON via a temp file or stdin per that script's documented interface) so they land in the `Job` table tagged `source: "jobdatalake"` alongside the curated per-company scrapes.

3. **Run the engine.** `npx tsx scripts/run-auto-apply.ts $ARGUMENTS` (pass through company names / `--limit` / `--max-minutes` as given). This single script call does everything: scrapes Greenhouse/Lever/Ashby for the requested companies, computes the priority-ranked eligible queue, runs the fill engine per job (bot-wall/CAPTCHA detection, multi-step forms, combobox handling, the per-company circuit breaker, hard timeouts — all handled inside the script, not by you), records every `Application` row, and prints a JSON summary to stdout.

4. **Report the summary** to the user in plain language: total scanned, how many were fillable vs `manual_apply_needed` (unsupported ATS — Workday and similar), the breakdown by status (`ready_for_review` / `submitted` / `failed`) and by company, and call out any company the circuit breaker just blocked (3 consecutive bot-detection/timeout/stuck-form failures) so the user knows to investigate manually via `--unblock` later. Point them at `/applications` in the dashboard (`npm run dev`) to review screenshots, and remind them that nothing gets submitted for a company until they run `/auto-apply --approve "Company Name"`.
