...
export async function scanFields(page: Page): Promise<DetectedField[]> {
  const raw = await page.evaluate(`(${[
    ...
    "    // Only real DOM signals count — Greenhouse assigns question_NNNN ids to",
    "    // EVERY custom question (plain text ones included, e.g. LinkedIn/Website),",
    "    // so id-pattern guessing produced false positives that silently failed to fill.",
    ...
    '    const looksLikeCombo = hasPopup || el.readOnly || nearSelectClass || /^(country|location)$/.test(fieldId);',
    "    results.push({",
